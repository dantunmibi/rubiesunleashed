        ...
        if (data && isMounted) {
            ...
            if (finalCount > 0) {
              ...
              if (wishData && wishData.length > 0) {
                ...
                if (allGames && allGames.length > 0) {
                      ...
                      return (
                        allGames.find(
                          (g) =>
                            ...
                            (g.original_blogger_id &&
                              g.original_blogger_id === item.game_id),
                        ) || null
                      );
                    })
                    .filter(Boolean);
                  ...
                  console.log(
                    "✅ Valid items loaded from unified feed:",
                    validGames.length,
                  );
                } else {
                  // ✅ Fallback: individual Blogger lookups if feed is empty
                  console.log(
                    "⚠️ Unified feed empty, falling back to individual lookups",
                  );
                  ...
                  console.log(
                    "✅ Valid items loaded from fallback:",
                    validGames.length,
                  );
                }
          ...
          if (data.role === "architect") {
            ...
            const { count: projectCount } = await supabase
              ...
              .select("*", { count: "exact", head: true })
              .eq("uploader_username", data.username);
  ...
  if (error || !profile) {
    return (
      <div className="min-h-screen bg-[#0b0f19] flex flex-col items-center justify-center text-slate-500 gap-4">
        ...
        <Link
          href="/"
          className="mt-4 px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-full text-xs font-bold uppercase tracking-widest transition-colors"
        >
          Return to Base
        </Link>
      </div>
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-6 text-center">
          <div className="bg-[#161b2c] border border-white/10 p-10 rounded-3xl max-w-md w-full shadow-2xl">
            ...
            <p className="text-slate-400">
              @{profile.username} has restricted access to their profile.
            </p>
          </div>
        </main>
        <Footer />
            ...
            <div className="flex-1 text-center md:text-left space-y-1 pb-2">
              <div className="flex items-center justify-center md:justify-start gap-3 flex-wrap">
                ...
                {isOwner && (
                  <Link
                    href="/settings"
                    className="p-2 text-slate-400 hover:text-white transition-colors bg-black/40 backdrop-blur-md rounded-full border border-white/10 hover:border-white/30"
                  >
                    <Edit size={16} />
                  </Link>
                )}
          ...
          <div className="space-y-6">
            ...
            <p className="text-slate-300 leading-relaxed text-lg font-light border-l-2 border-(--user-accent)/30 pl-6 italic">
              ...
              {isOwner && (
                <Link
                  href="/settings"
                  className="float-right ml-4 text-slate-400 hover:text-white transition-colors bg-black/40 backdrop-blur-md rounded-full border border-white/10 hover:border-white/30 p-1"
                >
                  <Edit size={16} />
                </Link>
              )}
            ...
            <div className="flex items-center justify-between mb-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {publishedCount}{" "}
                    {publishedCount === 1 ? "project" : "projects"} available
                  </p>
                </div>
              </div>
              {publishedCount > 0 && (
