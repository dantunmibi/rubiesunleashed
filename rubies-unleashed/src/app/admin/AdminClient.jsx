...
const handleModerate = async (actionType) => {
    ...
    setModerating(true);
    try {
      // ✅ Force refresh session to guarantee a valid token
      // getSession() can return stale/null — refreshSession() forces a new token
      let token = null;
      ...
      // ✅ Support both report objects (game_id) and project objects (id)
      const projectId = moderationModal.game_id || moderationModal.id;

      if (!projectId) {
        throw new Error("Could not determine project ID from modal context.");
      }
    ...
    <div className="min-h-screen bg-[#0b0f19] text-white font-sans selection:bg-red-500/30">
      ...
      <main className="pt-32 px-6 max-w-350 mx-auto pb-20">
            ...
            <div className="flex items-center gap-4 mb-3">
              ...
              <div>
                ...
                <p className="text-slate-400 text-sm font-medium mt-1">
                  Platform Oversight & Moderation Dashboard
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#161b2c]/50 backdrop-blur-xl border border-white/5 rounded-3xl p-8 min-h-125 shadow-2xl">
          ...
          {activeTab === "projects" && (
            <RecentProjectsTab setCommentModal={setCommentModal} />
          )}
...
const [reportsRes, bannedRes, usersRes, unifiedFeed, draftsRes] = await Promise.all([
  supabase.from("reports").select("id", { count: "exact", head: true }),
  supabase.from("projects").select("id", { count: "exact", head: true }).eq("status", "banned"),
  supabase.from("profiles").select("id", { count: "exact", head: true }),
  getUnifiedFeed({ limit: 2000 }),
...
setStats({
  ...
  return (
    <div className="flex gap-4">
      ...
      <StatBadge label="Published" value={stats.projects} color="emerald" />
      <StatBadge label="Drafts" value={stats.drafts} color="grey" />
    </div>
  );
}
...
function StatBadge({ label, value, color }) {
  const colors = {
    ...
    grey: "bg-slate-500/10 text-slate-400 border-slate-500/20",
  };

  return (
    <div
      className={`px-4 py-2 rounded-xl border ${colors[color]} backdrop-blur-sm`}
    >
...
function ModerationLog({
  ...
  const fetchActions = async () => {
    ...
    try {
      ...
      const [moderateRes, commentsRes] = await Promise.all([
      ...
      if (moderateRes.ok) {
        const { actions: data } = await moderateRes.json();
        setActions(data || []);
      }
      ...
      if (!commentsRes.error) {
        ...
        console.error("Failed to fetch comments:", commentsRes.error);
      }
    } catch (error) {
  ...
  const handleDeleteComment = async (commentId) => {
    ...
    try {
      ...
      console.log("✅ Comment record deleted");
    } catch (error) {
  ...
  const reviewRequests = actions.filter(
    (a) => a.action_type === "review_request",
  );
  ...
  if (loading)
    return (
      <Loader2 className="animate-spin mx-auto mt-10 text-red-500" size={32} />
    );
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={async () => {
            if (
              !confirm(
                "⚠️ Clear ALL moderation history? This cannot be undone!",
              )
            )
              return;

            try {
              // ✅ Always get a fresh session to avoid stale token 401s
              let session = null;
              try {
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Requested by {request.developer_username} •{" "}
                ...
                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      const projectId =
                        request.project_id ||
                        request.metadata?.original_hide_action_id;
                      if (projectId) {
                        setModerationModal({
                          game_id: projectId,
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        setActions((prev) =>
                          prev.filter((action) => action.id !== request.id),
                        );
      ...
      <div className="space-y-4">
        {filteredActions.length === 0 ? (
          ...
          filteredActions.map((action) => (
            <div
              key={action.id}
              className="bg-black/20 border border-white/5 rounded-xl p-6 hover:bg-black/30 transition-colors"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span
                      className={`px-3 py-1 rounded-lg border text-xs font-bold uppercase ${actionColors[action.action_type]}`}
                    >
                      {action.action_type}
                    </span>
      ...
      <div className="space-y-4 mt-10">
        <div className="flex items-center justify-between border-t border-white/5 pt-8">
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            ...
            Sent Comments ({comments.length})
          </h3>
        </div>

        {comments.length === 0 ? (
          <p className="text-center text-slate-500 py-10">
            No comments sent yet.
          </p>
        ) : (
          comments.map((c) => (
            <div
  ...
  const fetchProjectDetails = async (reports) => {
    ...
    if (bloggerIds.length > 0) {
      try {
        ...
        const allGames = await getUnifiedFeed({
          limit: 500,
          includeHidden: true,
        }); // ✅ Admin mode

        // Filter to only Blogger games
        const bloggerGames = allGames.filter(
          (game) => game.source !== "supabase",
        );
...
function UserManager() {
  ...
  const demoteUser = async (id) => {
    ...
    const { error } = await supabase
      .from("profiles")
      .update({ role: "user", archetype: "hunter" })
      .eq("id", id);
    if (error) alert("Error: " + error.message);
    else fetchUsers(search);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {users.map((u) => (
          <div
            key={u.id}
            className="flex items-center justify-between p-4 border border-white/5 rounded-xl bg-black/20 hover:bg-black/30 transition-colors"
          >
            <div className="flex items-center gap-3">
...
function ContentManager() {
  ...
  const fetchBannedProjects = async () => {
    ...
    try {
      ...
      const manuallyBannedIds = new Set(
        moderationActions?.map((a) => a.project_id) || [],
      );

      // Filter: Only show projects that HAVE a moderation action
      const manuallyBanned =
        projects?.filter((p) => manuallyBannedIds.has(p.id)) || [];

      console.log(
        "✅ Manually banned (has moderation_action):",
        manuallyBanned,
      );
  ...
  const permanentlyDelete = async (projectId, projectTitle) => {
    ...
    if (
      !confirm(
        `⚠️ PERMANENTLY DELETE "${projectTitle}"?\n\nThis action cannot be undone!`,
      )
    ) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `/api/projects/delete?id=${projectId}&hard=true`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${session?.access_token}`,
  ...
  return (
    <div className="space-y-8">
        ...
        <>
          ...
          <div>
            <h3 className="font-bold text-white mb-4 flex items-center gap-2">
              <EyeOff size={18} className="text-amber-500" /> Hidden Content (
              {hidden.length})
            </h3>
            <div className="space-y-3">
              {hidden.map((h) => (
      ...
      {contentTab === "banned" && (
        <div className="space-y-4">
          <h3 className="font-bold text-white flex items-center gap-2">
            <ShieldAlert size={18} className="text-red-500" /> Moderation Bans (
            {bannedProjects.length})
          </h3>
          <p className="text-slate-400 text-sm">
            Projects banned by admin action via moderation API.
          </p>
          ...
          {bannedProjects.length === 0 ? (
            ...
            bannedProjects.map((project) => (
              <div
                key={project.id}
                className="bg-red-900/10 border border-red-500/20 rounded-xl p-4 flex items-center justify-between"
              >
      ...
      {contentTab === "flagged" && (
        <div className="space-y-4">
          <h3 className="font-bold text-white flex items-center gap-2">
            <AlertTriangle size={18} className="text-amber-500" /> Auto-Flagged
            Projects ({flaggedProjects.length})
          </h3>
          <p className="text-slate-400 text-sm">
            Projects automatically flagged by 3+ malware reports.
          </p>
          ...
          {flaggedProjects.length === 0 ? (
            ...
            flaggedProjects.map((project) => (
              <div
                key={project.id}
                className="bg-amber-900/10 border border-amber-500/20 rounded-xl p-4 flex items-center justify-between"
              >
                <div className="flex items-center gap-4">
                  <img
                    src={project.cover_url || "/placeholder-game.png"}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {projects.map((project) => (
              <div
                key={project.id}
                className="bg-black/20 border border-white/5 rounded-lg p-4 flex items-center justify-between"
              >
                <div className="flex items-center gap-4">
                  <img
                    src={project.cover_url || "/placeholder-game.png"}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={() =>
                      toggleEditorsChoice(project.id, project.is_editors_choice)
                    }
                    className={`p-2 rounded transition-colors ${
                      project.is_editors_choice
...
function SystemManager() {
  ...
  const enriched = services.map((s) => ({
    ...s,
    ...SERVICES.find((def) => def.id === s.serviceId),
  }));
...
function BloggerMigration() {
  ...
  const fetchBloggerData = async () => {
    ...
    try {
      ...
      const claimedIds = new Set(
        claimed?.map((p) => p.original_blogger_id) || [],
      );
      const unclaimedPosts = bloggerData.filter(
        (post) => !claimedIds.has(post.id),
      );
  ...
  const searchUsers = async (query) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .or(
          `username.ilike.%${query}%,display_name.ilike.%${query}%,developer_name.ilike.%${query}%`,
        )
        .limit(10);
  ...
  const assignPosts = async (userId, username) => {
    ...
    try {
      ...
      alert(
        `✅ Successfully assigned ${result.assigned_count} posts to ${username}`,
      );
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">
            {bloggerPosts.length} unclaimed posts • {selectedPosts.size}{" "}
            selected
          </p>
        </div>
      ...
      {/* Developer Groups */}
      <div className="space-y-4">
        {Object.entries(groupedByDeveloper).map(
          ([developer, posts], devIndex) => (
            <div
              key={`dev-${devIndex}-${developer}`}
...
function PostCard({ post, selected, onToggle }) {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div>
        <h5 className="font-bold text-white text-sm mb-1 line-clamp-2">
          {postTitle}
        </h5>
      ...
      <div className="p-4 space-y-3">
        <div>
          <div className="flex items-start justify-between gap-2 mb-1">
            <h4 className="font-bold text-white line-clamp-1 flex-1">
              {project.title}
            </h4>
