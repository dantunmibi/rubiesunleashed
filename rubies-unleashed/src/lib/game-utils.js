...
export const isApp = (tags) => {
  ...
  return tags.some((t) =>
    ["app", "pwa", "tool", "utility", "software"].includes(t.toLowerCase()),
  );
};
...
const TAG_STYLES = {
  ...
  Featured:
    "text-white bg-ruby/20 border-ruby/50 shadow-[0_0_10px_rgba(224,17,95,0.3)]",
  DEFAULT: "text-slate-300 bg-white/5 border-white/10 hover:bg-white/10",
};

export const getTagStyle = (tag) => {
  const key = Object.keys(TAG_STYLES).find(
...
export const getLicenseDisplay = (license) => {
  const licenseMap = {
    Free: { text: "Free", color: "text-emerald-400" },
    Demo: { text: "Demo", color: "text-yellow-400" },
    Paid: { text: "Paid", color: "text-blue-400" },
    "Open Source": { text: "Open Source", color: "text-purple-400" },
  };
  return licenseMap[license] || licenseMap["Free"];
};
...
export const getPlatformInfo = (game, tags) => {
  ...
  if (game && game.buildPlatform && game.buildPlatform.trim().length > 0) {
    ...
    if (isWin && isMac)
      return { name: "PC / Mac", icon: <Monitor size={20} />, ver: "Multi" };
    if (isWin && isLinux)
      return { name: "PC / Linux", icon: <Monitor size={20} />, ver: "Multi" };
    if (isMac && isLinux)
      return { name: "Mac / Linux", icon: <Monitor size={20} />, ver: "Multi" };
    if (isAndroid && isWeb)
      return {
        name: "Android / Web",
  ...
  if (
    lowerTags.includes("web") ||
    lowerTags.includes("html5") ||
    lowerTags.includes("browser")
  ) {
    return { name: "Web Browser", icon: <Globe size={20} />, ver: "Any" };
  }
...
export const getDownloadIcon = (platform) => {
  ...
  if (lower.includes("mac") || lower.includes("apple") || lower.includes("osx"))
...
export const getSocialIcon = (label) => {
  ...
  if (lower.includes("mail") || lower.includes("contact"))
    return <Mail size={16} />;
  if (lower.includes("ko-fi") || lower.includes("patreon"))
    return <Coffee size={16} />;
  if (lower.includes("soundtrack") || lower.includes("music"))
    return <Music size={16} />;
  if (lower.includes("website") || lower.includes("itch"))
...
export const getDownloadLabel = (platform, tags = [], url = "") => {
  ...
  if (
    ...
    urlLower.includes("store.steampowered")
  )
  ...
  if (lower === "chrome web store" || lower === "chrome webstore")
    return "Get Extension";
  if (lower === "firefox addons" || lower === "firefox add-ons")
    return "Get Extension";
  if (lower === "edge addons" || lower === "edge add-ons")
    return "Get Extension";
  ...
  // Consoles
  if (lower === "nintendo switch" || lower === "switch")
    return "Get for Switch";
  ...
  if (
    ...
    const isExtensionProject =
      tagString.includes("extension") ||
    ...
    if (isWebApp) return "Try It Now"; // ✅ UPDATED
...
export function processSupabaseProject(row) {
  ...
  try {
    ...
    const downloadLinks =
      typeof row.download_links === "string"
    ...
    if (
      row.content_warning &&
      typeof row.content_warning === "string" &&
      row.content_warning.trim().length > 0
    ) {
    ...
    // Determine primary download URL
    const primaryDownload =
      row.download_url ||
      (downloadLinks.length > 0 ? downloadLinks[0].url : null);

    // Normalize Tags
    const tags = Array.isArray(row.tags) ? row.tags : [];

    // ✅ NEW: License with tag fallback
    let license = row.license || "Free";
    ...
    return {
      ...
      images: row.screenshots || [],
      screenshots: row.screenshots || [],
      videoUrl: row.video_url,
      video: row.video_url,
      html5_embed_url: row.html5_embed_url,
      license: license, // ✅ Now uses fallback detection
      ...
      downloadLinks:
        downloadLinks.length > 0
          ? downloadLinks
          : primaryDownload
            ? [{ platform: row.platform || "Unknown", url: primaryDownload }]
            : [],
      downloadUrl: primaryDownload,
      platforms: row.platform ? [row.platform] : [],
      buildPlatform: row.platform || "Multi-Platform",
...
export const PROJECT_STATUSES = [
  ...
  },
  {
    ...
    label: "Coming Soon",
    tag: "Coming Soon",
    description: "Not yet released — announce early",
  },
];
...
export function detectProjectStatus(tags) {
  ...
  if (tagString.includes('coming soon')) return 'coming_soon';
...
export function getBetaBadge(tags) {
  ...
  if (tagString.includes("coming soon")) {
    return { label: "Coming Soon", color: "violet", icon: "clock" };
  }
...
export function injectStatusTag(currentTags, newStatus) {
  const STATUS_TAGS = ["Beta", "Early Access", "Alpha", "Demo", "Coming Soon"];

  // Remove all existing status tags
  let cleanedTags = (currentTags || []).filter(
    (tag) => !STATUS_TAGS.includes(tag),
  );
