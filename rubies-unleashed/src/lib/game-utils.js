...
const GENRE_PRIORITY = [
  "App", "PWA", "Tool", "Utility", "Software", // ✅ NEW: App Types High Priority
...
export const getSmartTag = (tags) => {
  if (!tags || !Array.isArray(tags) || tags.length === 0) return "Item"; // Changed "Indie" to "Item"
...
// ... (Rest of file unchanged: getLicenseType, getPlatformInfo, getDownloadIcon, getSocialIcon) ...
...
export const getPlatformInfo = (game, tags) => {
  ...
  if (lowerTags.includes("mac") || lowerTags.includes("osx")) return { name: "Mac", icon: <Laptop size={20} />, ver: "Latest" };
  if (lowerTags.includes("pc") || lowerTags.includes("windows")) return { name: "Windows", icon: <Monitor size={20} />, ver: "10/11" };
  if (lowerTags.includes("android") || lowerTags.includes("apk")) return { name: "Android", icon: <Smartphone size={20} />, ver: "8.0+" };
  if (lowerTags.includes("web") || lowerTags.includes("html5")) return { name: "Web Browser", icon: <Globe size={20} />, ver: "Any" };
  return { name: "Universal", icon: <Globe size={20} />, ver: "Any" };
};
